...
#if defined(NDEBUG)
# error "Koruna cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc6d9f3941d5c5d8d901ac1f2bee51e7d2231a54c978d1ded075829d7b68ce79b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Koruna: starting difficulty is 1 / 2^12
...
{
    // Koruna: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 840000/20000 blocks, which will occur approximately every 4 years/year
    nSubsidy >>= (nHeight / 20000); // Koruna: 20k blocks in ~1 year

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Koruna: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Koruna: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Koruna: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Koruna: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa1ed63ae07103bc57677947bd52576168049b2ce014d9ffb3ddae695dda02096");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "mys Canaveral, Falcon Heavy uspesny vzlet a navrat";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04440a843625dd0a051f75609cf27f33bf27907f82251bbc4fb7c99320b4a9beb59f7c082b1bd0d5836c2712af7a4a7138159c94bc240cf85bd618b5a1dba0af04") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1518001632;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087531460;

        if (fTestNet)
        {
            block.nTime    = 1518001581;
            block.nNonce   = 385559891;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x7354511d28d98c279efd3c410fedfe5affeb91a9105b51961cbb8290f294021b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xca, 0xba, 0xda }; // Koruna: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KorunaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KorunaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KorunaMiner(CWallet *pwallet)
{
    printf("KorunaMiner started\n");
    ...
    try { loop {
        ...
        printf("Running KorunaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KorunaMiner, pwallet));
}
